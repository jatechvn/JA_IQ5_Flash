...
void main() {
  group('FirmwareSlotProfile Tests', () {
    test('Initializes 3 slot types with proper badges, icons, and dynamic i18n', () {
      setLang('EN');
    ...
    test('Validates directory with required Qualcomm files', () {
      ...
      try {
        ...
        File('${tempDir.path}/rawprogram_unsparse0.xml').writeAsStringSync('<xml/>');
        File('${tempDir.path}/patch0.xml').writeAsStringSync('<xml/>');
        File('${tempDir.path}/prog_firehose_ddr.elf').writeAsStringSync('binary');
  ...
  group('Bento Glassmorphism UI Component Tests', () {
                ...
                child: Column(
                  ...
                  children: [
                    PillBadge(
                      ...
                      border: win11DarkColors.accentEmerald.withValues(alpha: 0.3),
                      showDot: true,
                    ),
    ...
    test('FirmwareSlotProfile supports dynamic custom naming and localization', () {
      setLang('VI');
      final slot = FirmwareSlotProfile(
        index: 0,
        type: FirmwareSlotType.factory,
      ...
      expect(slot.lastValidation.localizedMessage, equals('No folder selected'));
    });
  });
  ...
  group('Settings & Glassmorphism Tuning Tests', () {
    ...
    testWidgets('Renders showSettingsDialog with all 4 tabs and interactions', (
      tester,
    ) async {
      final theme = AppTheme();
      setLang('EN');
    ...
    testWidgets('TopBarExpandingButton renders collapsed and expands on hover', (
      tester,
    ) async {
      bool tapped = false;
      await tester.pumpWidget(
  ...
  group('Path Input and Browse i18n Tests', () {
    ...
    test('Trims and strips double and single quotes from pasted paths', () {
      ...
      expect(sanitizePath('  "D:\\Firmware\\IQ5_v1.0"  '), equals('D:\\Firmware\\IQ5_v1.0'));
      expect(sanitizePath("'D:\\Firmware\\IQ5_v2.0'"), equals('D:\\Firmware\\IQ5_v2.0'));
  ...
  group('UI Polish & Refinements Tests (Sprint 5)', () {
    ...
    testWidgets('TopBarExpandingButton uses vector translate icon without text emojis', (
      tester,
    ) async {
  ...
  group('RotatingGlowBorder Component Tests', () {
    ...
    testWidgets('RotatingGlowBorder activates CustomPaint and animates when active', (
      tester,
    ) async {
      await tester.pumpWidget(
        const MaterialApp(
    ...
    testWidgets('RotatingGlowBorder dynamically responds to isActive toggling', (
      tester,
    ) async {
      bool active = false;
    ...
    test('RotatingGlowBorderPainter paints on canvas and handles shouldRepaint', () {
      ...
      expect(
        () => painter1.paint(canvas, Size.zero),
        returnsNormally,
      );
  ...
  group('Hardware Tier & Auto-Profiling Tests', () {
    ...
    testWidgets('Settings dialog renders Hardware Profile Section & switches tiers', (
      tester,
    ) async {
      final theme = AppTheme();
      setLang('EN');
  ...
  group('BounceMarquee & GlassBouncePathField Tests', () {
    testWidgets('BounceMarqueeText renders text in horizontal scroll view', (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: SizedBox(
              ...
              child: BounceMarqueeText(
                text: 'D:\\Very\\Long\\Firmware\\Directory\\Path\\That\\Exceeds\\Width',
              ),
            ),
          ),
        ),
      ...
      expect(
        find.text('D:\\Very\\Long\\Firmware\\Directory\\Path\\That\\Exceeds\\Width'),
        findsOneWidget,
      );
    ...
    testWidgets('GlassBouncePathField shows BounceMarqueeText when not focused and text is present', (
      tester,
    ) async {
      ...
      expect(textField.controller?.text, equals('C:\\Firmware\\Stock_Qualcomm_Fastboot_v10.3'));
    });

    testWidgets('GlassBouncePathField displays TextField directly when text is empty', (
      tester,
    ) async {
      ...
      expect(find.text('Paste or browse firmware directory...'), findsOneWidget);
    });

    test('Light Mode Orb colors are soft pastels to prevent dark card blotches', () {
      // win11LightColors
