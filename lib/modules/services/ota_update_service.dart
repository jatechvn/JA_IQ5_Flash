...
class SemanticVersion implements Comparable<SemanticVersion> {
  ...
  /// Phân tích cú pháp chuỗi phiên bản dạng: '1.2.1', 'v1.2.1', '1.2.1+2', '1.3.0-beta'
...
class OtaUpdateConfig {
  ...
  factory OtaUpdateConfig.defaults() => const OtaUpdateConfig(
    serverPath: r'\\10.81.141.226\temp\FBT\JA_PROJECT\JA_Update\JA_IQ5_Flash',
    username: 'user',
    password: 'user',
    checkInterval: 'daily',
    autoDownload: false,
...
class OtaUpdateService {
  static bool isValidPackageName(String name) =>
      RegExp(
        r'^JA_IQ5_Flash_[a-zA-Z0-9_.+-]+\.zip$',
        caseSensitive: false,
      ).hasMatch(name) &&
      !name.contains('..');

  static final RegExp _packageVersionPattern = RegExp(
    r'^JA_IQ5_Flash_[vV]?(\d+\.\d+(?:\.\d+)?(?:-[a-zA-Z0-9.-]+)?(?:\+\d+)?)(?:_[a-zA-Z0-9_]+)?\.zip$',
    caseSensitive: false,
  );

  static SemanticVersion? packageVersionFromFileName(String fileName) {
    if (!isValidPackageName(fileName)) return null;
    return SemanticVersion.tryParse(
      _packageVersionPattern.firstMatch(fileName)?.group(1),
    );
  }
  ...
  /// 2. Thư mục AppData (%APPDATA%\JA_IQ5_Flash\update_config.json)
  File getConfigFile() {
    ...
    if (appData != null && appData.isNotEmpty) {
      final dir = Directory('$appData\\JA_IQ5_Flash');
      if (!dir.existsSync()) {
        try {
  ...
  Future<UpdateCheckResult> checkForUpdates({
    ...
    // 1. Kết nối máy chủ
    final connected = await connectSmbShare(
      path: serverPath,
    ...
    await saveExternalConfigFile(_cachedConfig.copyWith(lastCheckTime: now));
    ...
    if (await versionJsonFile.exists()) {
      try {
        ...
        final packageSemVer = fileName == null
            ? null
            : packageVersionFromFileName(fileName);
        if (serverSemVer != null &&
            fileName != null &&
            packageSemVer != null &&
            serverSemVer == packageSemVer &&
            (checksum == null ||
    ...
    try {
      ...
      // vd: JA_IQ5_Flash_v1.2.2_Windows_x64.zip -> 1.2.2
  ...
  Future<Directory> _performUpdate(
    ...
    if (packageInfo.sha256 != null) {
      final actualChecksum = await sha256.bind(localZipFile.openRead()).first;
      if (actualChecksum.toString() != packageInfo.sha256) {
        throw StateError('Update package checksum verification failed');
      }
    }

    // 2. Giải nén gói cập nhật với cơ chế chống Zip-Slip
...
""");
    ...
    // Nếu zip đóng gói lồng 1 thư mục gốc (vd: JA_IQ5_Flash_v1.2.2_Windows_x64)
    final subDirs = extractDir.listSync().whereType<Directory>().toList();
    if (subDirs.length == 1) {
      final testExe = File('${subDirs.first.path}\\ja_iq5_flash.exe');
      if (testExe.existsSync()) {
        payloadDir = subDirs.first;
      }
    }

    for (final name in ['ja_iq5_flash.exe', 'flutter_windows.dll', 'data']) {
...
chcp 65001 >nul
title JA IQ5 Reflash - Dang Cap Nhat Phien Ban Moi...
...
echo ========================================================
echo   JA IQ5 REFLASH - DANG TIEN HANH CAP NHAT
echo ========================================================
echo.
...
robocopy "%DST_DIR%" "%BACKUP_DIR%" /E /NP /R:2 /W:1 /XD logs backups temp /XF config.ini update_config.json license.key >"%~dp0backup.log"
if errorlevel 8 exit /b 13
robocopy "%SRC_DIR%" "%DST_DIR%" /E /IS /IT /NP /R:5 /W:2 /XD logs backups temp /XF config.ini update_config.json license.key >"%~dp0apply.log"
